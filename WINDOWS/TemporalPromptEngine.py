            ...
            for prompt_index, scene_description in enumerate(scene_descriptions, start=1):
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        detailed_prompt = (
                            f" Take {prompt_index}:{scene_description} and create a specific, pg-13 friendly, detailed paragraph long video prompt that focuses on the visual scene for it including all specific details and aspects of the scene. You are focusing on the visual aspects scene as if describing it to a blind person. Avoid abstract or emotional descriptions. It must be leaned towards american consumers, unless otherwise specificed direct prompts towards american actors and settings. Every prompt must terms to further show it is set in {video_options['decade']} decade\n"
                            ...
                            f" ALWAYS INCLUDE (Camera Language, Framing Angle, Lighting, Subject Description, Subject Movement, Scene Description, Atmosphere) in extensive detail and integrated seamlessly into the narrative, traditional exposition is not required, only visual description of the 6 second visual scene is required in temporal order. \n"
                            f"- YOU DO NOT use bullets, lists, or any formatting other than narrative paragraphs. Generate exactly one Positive and one Negative for this scene. There should never be more than one set per story generation. One set = One Positive Prompt + One Negative Prompt"
