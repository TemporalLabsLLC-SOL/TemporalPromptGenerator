...
INFERENCE_STEPS = [90]  # Adjusted for 5b model
SEED = 1990
...
def summarize_text(text: str, max_tokens: int = 220, min_tokens: int = 30) -> str:
    """
    Refines, expands, and truncates the input text to fit within the 220-token limit, preserving essential details
    and adhering to the specific needs of video prompts. The function breaks sentences if necessary to fit more details.

    Parameters:
    - text (str): The input text to expand and refine.
    - max_tokens (int): The maximum number of tokens allowed (default is 220 tokens).
    - min_tokens (int): The minimum number of tokens to preserve after refinement.

    Returns:
    - str: The refined, truncated, and detailed text adhering to the token limit.
    """
    if not tokenizer:
    ...
    # If the input is already within limits, expand details
    if num_tokens <= max_tokens:
        return add_details(text)

    # Prioritize key elements before truncating
    expanded_text = add_details(text)

    # Re-tokenize after expansion
    tokens_after_expansion = tokenizer.encode(expanded_text, return_tensors="pt").shape[1]

    # If the expanded text exceeds token limits, truncate while keeping core details
    if tokens_after_expansion > max_tokens:
        truncated_text = truncate_to_token_limit(expanded_text, max_tokens)
    else:
        truncated_text = expanded_text
...
def add_details(text: str) -> str:
    """
    Expands the input text by adding more specific details about characters, themes, lighting, and other visual elements,
    even if resulting in broken or less cohesive sentences to maximize detail retention.

    Parameters:
    - text (str): The original prompt to be expanded.
    ...
    # 2. Enhancing camera work
    if "camera" in text:
        text += " Professional ARRI Alexa LF. Slow sweeping movement. Alternating wide shots and close-ups of characters and environments."

    # 3. Expanding on lighting details
    if "lighting" in text:
        text += " Dynamic lighting shifts from soft ambient glow to cool shadows. Highlights key emotional moments and environment textures."

    # 4. Detailing the environment and textures
    if "environment" in text or "landscape" in text:
        text += " Texture of the environment: sleek metal, reflective surfaces, rough urban streets. Mist rises in the background. Futuristic setting."

    # 5. Enriching the theme and mood
    if "theme" in text or "mood" in text:
        text += " Theme blends mysticism with advanced technology. Every shot reinforces the duality of these concepts."
...
def main():
    ...
    for idx, prompt_data in enumerate(prompts, start=1):
        ...
        five_word_summary = create_five_word_summary(summarized_positive)

        # Combine summarized positive and negative prompts
        combined_prompt = f"positive: {summarized_positive}\nnegative: {summarized_negative}"
